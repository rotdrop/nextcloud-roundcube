...
Roundcube.iframe_loaded = function() {
    ...
    var roundcubeContentsMainscreen = roundcubeContents.find('#mainscreen'),
        roundcubeContentsToolbar = roundcubeContents.find('#messagetoolbar'),
        // remove header line, includes about line and
        roundcubeContentsTopline = roundcubeContents.find('#topline'),
    // correct top padding
        top_margin = 10,
        rc_version = '',
        rc_theme   = '';
    ...
    if ($('#disable_header_nav').val() === 'on') {
        ...
        // check if the above element exists (only in new larry theme, if null
        // use rc 0.7 default theme
